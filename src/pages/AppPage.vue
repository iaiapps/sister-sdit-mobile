            ...
            <div class="row g-2">
                ...
                <div class="col-6" v-for="menu in extraMenus" :key="menu.id">
                    <div class="app-card">
                        ...
                        <div class="foot">
                            <button v-if="menu.type === 'modal'" type="button" class="btn-pilih" data-bs-toggle="modal" data-bs-target="#modalshow" v-on:click="openModal(menu)">pilih</button>
                            <button v-else class="btn-pilih" v-on:click="openInaDynamic(menu)">pilih</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog vh-100 m-0 p-2">
                <div class="modal-content height">
                    ...
                    <div class="modal-body">
                        <iframe v-if="show" class="iframee holds-the-iframe" v-bind:src="url" title="modal"></iframe>
                    </div>
                </div>
            </div>
        </div>
...
const loadExtraMenus = async () => {
    ...
    try {
        const res = await axios.get(`${props.url}/mobile-menus.json`, { timeout: 5000 });
        const all = res.data || [];
        extraMenus.value = all.filter(m => m.id >= 5);
